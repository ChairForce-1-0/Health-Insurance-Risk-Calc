...
// Serve static files from 'public' directory (Ensure paths align with Azure setup)
...
app.post('/calculate-risk', (req, res) => {
    try {
        ...
        function CalcBMIPoints(BMI) {
            ...
            throw new Error("BMI score out of range");
        }
        ...
        function CalcSystolicBpPoints(systolic) {
            return systolic < 120 ? 0 : systolic < 130 ? 15 : systolic < 140 ? 30 : 75;
        }

        function CalcDiastolicBpPoints(diastolic) {
            return diastolic < 80 ? 0 : diastolic < 90 ? 15 : diastolic < 100 ? 30 : 75;
        }
        ...
        const { totalPoints, riskCategory } = CalcTotalPoints(
            agePoints, BMIPoints, systolicPoints, diastolicPoints, familyHistoryPoints
        );

        res.json({
            agePoints, BMI, BMIPoints, systolicPoints,
